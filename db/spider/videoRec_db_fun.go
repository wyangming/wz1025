...
func (this *VideoRecDbFun) SpiderVideoRecsSave(videoRecs *[]*model.SpiderVideoRec) {
	...
	db := db.GetDb()
	tx,err:=db.Begin()
	stmt_obj, err := tx.Prepare("INSERT INTO zj_video_rec (video_type,video_plot_type,video_place,video_spider_url,video_spider_update_date,video_name,video_title,video_down_urls) VALUES(?,?,?,?,?,?,?,?) ON DUPLICATE KEY UPDATE video_down_urls=?")
	if err != nil {
	...
	for _, videoRec := range *videoRecs {
		go func(tmp_videoRecs *model.SpiderVideoRec) {
			_, err := stmt_obj.Exec(tmp_videoRecs.VideoType, tmp_videoRecs.VideoPlotType, tmp_videoRecs.VideoPlace, tmp_videoRecs.VideoSpiderUrl, tmp_videoRecs.VideoSpiderUpdateDate, tmp_videoRecs.VideoName, tmp_videoRecs.VideoTitle, tmp_videoRecs.VideoDownUrls, tmp_videoRecs.VideoDownUrls)
			if err != nil {
